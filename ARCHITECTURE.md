...
**Field access:** `includes/db.php` provides `umtd_get_field()` as the read interface for scalar fields. It reads from custom tables first (with a static per-request row cache), falls back to `get_field()` for any field not yet intercepted on save, and logs fallback hits to `error_log` when `WP_DEBUG` is enabled. Agent names on works and events are read from junction table JOINs (`$wa->name_display`), not via per-agent `umtd_get_field()` calls. Field tables below reflect current ACF field names; the access layer is transparent to field name.
...
| `organizing_agents` | — | relationship | → `umtd_agents`; `return_format: object`; written to `umtd_event_agents` (role: curator) on save at priority 30; templates read via `umtd_get_event_agents()` |
| `participating_agents` | — | relationship | → `umtd_agents`; `return_format: object`; written to `umtd_event_agents` (role: artist) on save at priority 30; templates read via `umtd_get_event_agents()` |
| `description` | — | textarea | |
| `event_link` | — | url | |
...
**Agent display names in templates:** For an agent's own page, use `$agent['name_display']` from `umtd_get_agent()`. For agent names on works, use `$wa->name_display` from `umtd_get_work_agents()` (included in the JOIN). For agent names on events, use `$ea->name_display` from `umtd_get_event_agents()`. Never `get_the_title()`. Never `get_field('name_display', $id)` in template or part files — agent names are always served by the data layer.
...
```php
$agents = umtd_get_work_agents( $post_id ); // returns array of stdClass with agent_post_id, name_display, role_slug, role_label, sort_order
```
...
Query all agents for an event, with roles:

```php
$event_agents = umtd_get_event_agents( $post_id ); // returns array of stdClass with agent_post_id, name_display, role_slug, role_label
// Filter by role: save.php maps organizing_agents → 'curator', participating_agents → 'artist'
$organizers   = array_filter( $event_agents, fn( $ea ) => 'curator' === $ea->role_slug );
$participants = array_filter( $event_agents, fn( $ea ) => 'artist'  === $ea->role_slug );
```
...
```php
$work  = umtd_get_work( $post_id );   // all work fields; per-type fields gated on work_type_slug
$agent = umtd_get_agent( $post_id );  // all agent fields; no 'title' key — use 'name_display'; works list separate via umtd_get_agent_works()
$event = umtd_get_event( $post_id );  // all event scalar fields; agents via umtd_get_event_agents(), works via umtd_get_event_works()
```
...
│   ├── card-event.php      — receives $args['event'], $args['organizers'] (junction table rows with ->name_display)
│   └── work-type/
